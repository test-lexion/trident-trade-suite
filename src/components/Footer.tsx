...
export const Footer = () => {
  ...
  // The "Trade" item should be active for any of the trading pages.
  ...
  return (
    <footer className="sticky bottom-0 z-50 w-full border-t border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80 md:hidden">
      <nav className="container flex h-16 items-center justify-around px-2">
        {navItems.map((item) => {
          const isActive = item.path === "/" ? isTradeActive : location.pathname === item.path;
          return (
            <Link
              key={item.label}
              to={item.path}
              className={cn(
                "flex h-full w-full flex-col items-center justify-center gap-1 px-2 text-xs transition-colors",
                isActive
                  ? "text-primary"
