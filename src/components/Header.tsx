...
export const Header = () => {
  ...
  const [userAddress] = useState("0x7a3f...1a5e");
  ...
  const copyAddress = () => {
    navigator.clipboard.writeText("0x7a3f8b2e4c9d1a5e");
      ...
      <div className="container flex h-16 items-center justify-between px-4">
        <div className="flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            <Link to="/stats"><Button variant={isActive("/stats") ? "secondary" : "ghost"}>Stats</Button></Link>
            <Link to="/governance"><Button variant={isActive("/governance") ? "secondary" : "ghost"}>Governance</Button></Link>
          </nav>
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2">
                <span className="hidden font-mono text-sm sm:inline">{userAddress}</span>
                <Wallet className="h-4 w-4 sm:hidden" />
                <ChevronDown className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuItem asChild><Link to="/rewards" className="w-full cursor-pointer"><Gift className="mr-2 h-4 w-4" />My Rewards & Loyalty</Link></DropdownMenuItem>
              <DropdownMenuItem onClick={() => setWalletDialogOpen(true)}><Wallet className="mr-2 h-4 w-4" />Wallet Info</DropdownMenuItem>
              <DropdownMenuItem><BarChart3 className="mr-2 h-4 w-4" />Transaction History</DropdownMenuItem>
              <DropdownMenuSeparator />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 overflow-x-auto rounded bg-muted px-3 py-2 font-mono text-sm">{userAddress}</code>
                <Button size="sm" variant="outline" onClick={copyAddress}>
                  {addressCopied ? <CheckCircle className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4" />}
                </Button>
              </div>
            </div>
            <div className="rounded-lg border border-border p-4 space-y-3">
              ...
              <div className="space-y-2 pt-2 border-t border-border">
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">USDC</span><span className="font-mono-numeric">$25,000.00</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">BTC</span><span className="font-mono-numeric">$16,062.63</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">ETH</span><span className="font-mono-numeric">$4,104.90</span></div>
              </div>
            </div>
